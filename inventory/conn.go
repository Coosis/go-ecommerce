...
func ConnectValkeyWithBackoff(ctx context.Context, url string) (valkey.Client, error) {
	const (
		maxAttempts  = 10
		initialSleep = 500 * time.Millisecond
		maxSleep     = 10 * time.Second
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		// You can tune timeouts here as needed
		cli, err := valkey.NewClient(valkey.ClientOption{
			InitAddress: []string{url},
			// DialTimeout: time.Second * 3, // (optionally set)
			// WriteTimeout: time.Second * 3,
			// ReadTimeout:  time.Second * 3,
		})
		if err == nil {
			ctxPing, cancel := context.WithTimeout(ctx, 3*time.Second)
			res := cli.Do(ctxPing, cli.B().Ping().Build())

			if cmdErr := res.Error(); cmdErr == nil {
				pong, err := res.ToString()
				cancel()
				if err == nil {
					if pong == "PONG" {
						log.Info("Valkey ready (PING=PONG)")
						return cli, nil
					}
				}
			} else {
				cancel()
				log.Warnf("Valkey PING command error: %v", cmdErr)
			}
			cli.Close()
		}

		log.Warnf("Valkey not ready (attempt %d/%d): %v", attempt, maxAttempts, err)
		time.Sleep(sleep)
		sleep *= 2
		if sleep > maxSleep {
			sleep = maxSleep
